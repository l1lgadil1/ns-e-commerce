...
const TextParallaxContent = ({ imgUrl, subheading, heading, children, isFirst }:any) => (
  <div>
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const ExampleContent = ({ title, text, href }:{title:string, href:string, text:string}) => {
  const targetRef = useRef(null);
  const { scrollYProgress } = useScroll({
    target: targetRef,
...
export const ParallaxBox = ({ imgUrl, subHeading, heading, isFirst, href }:{imgUrl:string, href:string, subHeading:string, heading:string, isFirst:boolean}) => (
  <div>
    <TextParallaxContent
      imgUrl={imgUrl}
      ...
      isFirst={isFirst}
    >
      {heading && subHeading && <ExampleContent href={href} text={heading} title={subHeading} />}
    </TextParallaxContent>
  </div>
);
